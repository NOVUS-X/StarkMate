...
use contract::match_result_storage::{
    IMatchResultStorageDispatcher, IMatchResultStorageDispatcherTrait, MatchResult,
    MatchResultStorage,
};
...
fn deploy_store_result(oracle_address: ContractAddress) -> ContractAddress {
...
fn test_setup() {
    ...
    assert(store_result_address != 0.try_into().unwrap(), 'Contract deployed');
...
fn test_store_and_get_result() {
    ...
    let store_result_address = deploy_store_result(ORACLE());
    let store_result_dispatcher = IMatchResultStorageDispatcher {
        contract_address: store_result_address,
    };
