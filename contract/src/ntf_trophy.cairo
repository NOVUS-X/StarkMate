...
pub trait ISoulboundTrophy<TContractState> {
    fn create_trophy(
        ref self: TContractState, name: felt252, description: felt252, metadata_uri: felt252,
    ) -> u256;
...
pub mod SoulboundTrophy {
    ...
    use super::{ContractAddress, Trophy, TrophyDetails};
    ...
    #[constructor]
    fn constructor(
        ref self: ContractState, owner: ContractAddress, _name: felt252, _symbol: felt252,
    ) {
    ...
    impl SoulboundTrophyImpl of super::ISoulboundTrophy<ContractState> {
        fn create_trophy(
            ref self: ContractState, name: felt252, description: felt252, metadata_uri: felt252,
        ) -> u256 {
            self.ownable.assert_only_owner();

            let trophy_id = self.next_trophy_id.read();
            let trophy = Trophy { name, description, uri: metadata_uri };
        ...
        fn get_trophy_details(self: @ContractState, trophy_id: u256) -> TrophyDetails {
            let trophy = self._get_trophy(trophy_id);
            TrophyDetails { name: trophy.name, description: trophy.description, uri: trophy.uri }
        }
    }
    ...
    impl InternalImpl of InternalTrait {
        fn _get_trophy(self: @ContractState, trophy_id: u256) -> Trophy {
            assert(
                trophy_id > 0 && trophy_id < self.next_trophy_id.read(), 'Trophy does not exist',
            );
